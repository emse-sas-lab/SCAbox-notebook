...
#INVERSE SHIFT ROW
invSR = (0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11)

#SHIFT ROW
SR = (0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3)

class AESSelectionFct(object):
    """A class containing predefined AES selection functions"""

    def k_xor_m(k,m):
        return k ^ m 

    def ttable0_k_xor_m(k,m):
        return Te0[k ^ m]

    def ttable1_k_xor_m(k,m):
        return Te1[k ^ m]

    def ttable2_k_xor_m(k,m):
        return Te2[k ^ m]

    def ttable3_k_xor_m(k,m):
        return Te3[k ^ m]

    def sbox_k_xor_m(k,m):
        """ for first round AES attack"""
        return sbox_tab[k ^ m]

    def invsbox_k_xor_m(k,m):
        return invsbox_tab[k ^ m]

    def sbox_m(k,m):
        return sbox_tab[m]

    def sbox_k_xor_m___xor___m(k,m):
        return AESSelectionFct.sbox_k_xor_m(k,m) ^ m

    def sbox_k_xor_m___xor___k(k,m):
        return AESSelectionFct.sbox_k_xor_m(k,m) ^ k

    def sbox_k_xor_m___xor___k_xor_m(k,m):
        return AESSelectionFct.sbox_k_xor_m(k,m) ^ AESSelectionFct.k_xor_m(k,m)

    def invsbox_k_xor_m___xor___m(k,m):
        """ for last round AES attack"""
        return AESSelectionFct.invsbox_k_xor_m(k,m) ^ m

class AESUtility(object):
    """Utility AES Class containing methods for building complex selection functions"""

    def sbox(self,num):
        """Retrieves a given S-Box Value"""
        return sbox_tab[num]

    def invsbox(self,num):
        """Retrieves a given Inverted S-Box Value"""
        return invsbox_tab[num]
    ...
    def createRoundKey(self, expandedKey, roundKeyPointer):
        """Create a round key. Creates a round key from the given expanded key and the
        position within the expanded key.
        """
        roundKey = [0] * 16
        for i in range(16):
                roundKey[i] = expandedKey[roundKeyPointer + i]
        return roundKey
    ...
    def RoundKey(self,key_in, rank):
        """Calculate the round key at the given rank"""
        nr = 10
        expand = self._expandKey(key_in, 16, 16*(nr+1))
        k = self.createRoundKey(expand,rank*16)
        return k
...
def _inv_mix_columns(state:  List[List[bytes]]) -> list:
    ...
    """
    for s in state:
        u = _xtime(_xtime(s[0] ^ s[2]))
        v = _xtime(_xtime(s[1] ^ s[3]))
        s[0] ^= u
        s[1] ^= v
        s[2] ^= u
        s[3] ^= v
...
class AES:
    ...
    def encrypt(self, data: bytes, iv=None) -> tuple:
        """ Encrypts a single block of data using the AES 

        :param iv: Optional IV to use in CBC mode. If unset, a random IV will be used
        :param data: The data to encrypt
        :return: a tuple where the first element is the encrypted data. Under CBC mode, the second value is the IV used, under ECB mode, the second value is None
        """
